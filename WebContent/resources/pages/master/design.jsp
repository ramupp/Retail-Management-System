...
<script src="../../../resources/js/validation.js"></script> 
 ...
 <script>
            ...
            {
            	$.get('../../../resources/ajax/generateBarcode.jsp',{code:code}, function (response) {
                  //	alert(response.trim());
...
$(document).ready(function() {
 ...
 var p2="co_id";
 var selectedCol12="co_id,co_nm,outlet"; 
 CallDropDownServiceForConcat(params12,p2,selectedCol12);
 ...
 function HideLoader() {
  ...
  function getDesgNo(val)
  { //alert(val);
	  $.get('../../../resources/ajax/checkDuplicateDesg.jsp',{des_no:val}, function (response) {
	      var x=response.trim();	
		 // alert(x);
	       
	         if(parseInt(x)>0)
	        	 {
	        	 alert("This design Number already Exists");
	        	 $('#desgNo').val("");
	        	 $('#desgNo').focus();
	        	 }
  ...
  function displayRateCode(x)
  {
	  var m=$(x).closest("tr").find("#sale_type").val();
	  if(m=='CR')
		  {
		  $(x).closest("tr").find("#rCode").removeAttr('disabled');
		  }
	  else
		  {
		  $(x).closest("tr").find("#rCode").val("");
		  $(x).closest("tr").find("#rCode").prop('disabled',true); 
		  }
        ...
        { type: "control",itemTemplate: function(value, item) {
         ...
         var $customEditButton = $("<p>").attr({class: "customGridEditbutton fa fa-caret-square-o-right"})
           .click(function(e) {
            ...
            var x=Date.now();
             $.get('../../../resources/ajax/forDesignMasterView.jsp', {tr_id: item.tr_id,ttimestamps:x}, function (response)
                     {
                       // alert(response);
...
<center>
    ...
    <li class="nav-item">
       ...
       <div class="tab-content">
				...
				<td>
				 <input type="text" name="desg_no" id="desgNo" class="form-control"  style=" width: 75%;" onchange="getDesgNo(this.value)" />
...
<!--                     </tr> -->
                    ...
                    <td><input type="text" name="rCode[]" id="rCode" placeholder="Rate Code" />
                    <td><input type="text" name="rate[]" id="rate" style=" width: 80px; text-align:right" />
                    <td><input type="text" name="remarks[]" id="remarks" style=" width: 60px; text-align:right" />
                   ...
                   <select name="default_rate[]" id="default_rate" class="browser-default">
                        ...
                        <option value='Y'>Yes</option>           
                            ...
                            <input type="hidden" name="tr_ids[]" id="tr_ids" />
                   </td>
                    </tr>
